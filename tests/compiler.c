...
#define DEFINE_AST_HELPER(T_, K_)\
...
{ \
  ...
  let->expr = jkl_node_new(JKL_NODE_INT); \
  let->id->value.s = name; \
  let->expr->value.K_ = value; \
  return let; \
}

DEFINE_AST_HELPER(qword, i)
DEFINE_AST_HELPER(string, s)

START_TEST(test_jkl_compile_check_let)
{
  jkl_program_t *program = jkl_program_new();
...
Suite *jkl_compiler_suite()
{
  ...
  tcase_add_test(tc_core, test_jkl_compile_check_let);
